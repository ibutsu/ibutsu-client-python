    ...
    def set_attribute(self, name, value):
        ...
        if name in self.openapi_types:
            ...
            raise ApiAttributeError(
                f"{type(self).__name__} has no attribute '{name}'", path_to_item
            )
...
class ModelComposed(OpenApiModel):
    ...
    def get(self, name, default=None):
        ...
        if len_values == 0:
            ...
            raise ApiValueError(
                f"Values stored for property {name} in {type(self).__name__} differ when looking "
...
def check_validations(validations, input_variable_path, input_values, configuration=None):
    ...
    if (
        ...
        if flags != 0:
            ...
            err_msg = rf"{err_msg} with flags=`{flags}`"
...
def remove_uncoercible(
    ...
    for required_type_class in required_types_classes:
        ...
        if (
            must_convert and class_pair in COERCIBLE_TYPE_PAIRS[spec_property_naming]
...
def deserialize_primitive(data, klass, path_to_item):
    ...
    try:
        ...
        raise ApiValueError(
            f"{additional_message}Failed to parse {data!r} as {klass.__name__}",
...
def type_error_message(var_value=None, var_name=None, valid_classes=None, key_type=None):
    ...
    msg = (
        f"Invalid type for variable '{var_name}'. Required {key_or_value} type {valid_classes_phrase} and "
        f"passed type was {type(var_value).__name__}"
    )
    return msg
