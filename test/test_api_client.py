...
class TestParametersToTuples:
    ...
    def test_parameters_to_tuples_none_params(self):
        """Test parameters_to_tuples with None params raises TypeError."""
        client = ApiClient()
        # None params are not supported and will raise TypeError
        with pytest.raises(TypeError):
            client.parameters_to_tuples(None, None)

    def test_parameters_to_tuples_empty_dict(self):
        """Test parameters_to_tuples with empty dict."""
        client = ApiClient()
        result = client.parameters_to_tuples({}, None)
        assert result == []

    def test_parameters_to_tuples_none_values(self):
        """Test parameters_to_tuples with None values."""
        client = ApiClient()
        params = {"key1": "value1", "key2": None, "key3": "value3"}
        result = client.parameters_to_tuples(params, None)
        # Should include None values as-is
        assert ("key1", "value1") in result
...
class TestParametersToUrlQuery:
    ...
    def test_parameters_to_url_query_none_params(self):
        """Test parameters_to_url_query with None params raises TypeError."""
        client = ApiClient()
        # None params are not supported and will raise TypeError
        with pytest.raises(TypeError):
            client.parameters_to_url_query(None, None)

    def test_parameters_to_url_query_empty_dict(self):
        """Test parameters_to_url_query with empty dict."""
        client = ApiClient()
        result = client.parameters_to_url_query({}, None)
        assert result == ""

    def test_parameters_to_url_query_none_values(self):
        """Test parameters_to_url_query with None values."""
        client = ApiClient()
        params = {"q": None, "page": 1, "filter": None}
        result = client.parameters_to_url_query(params, None)
        # Should include page but handle None values appropriately
        assert "page=1" in result
        # None values should be converted to string "None" or skipped
...
class TestFilesParameters:
    ...
    def test_files_parameters_from_pathlib_path(self, tmp_path):
        """Test files parameters from pathlib.Path object converted to string."""
        client = ApiClient()
        test_file = tmp_path / "test.txt"
        test_file.write_bytes(b"test content from Path")

        # Path objects need to be converted to string for files_parameters
        # This documents that str(Path) is the expected usage
        result = client.files_parameters({"file": str(test_file)})
        assert len(result) == 1
        assert result[0][0] == "file"
        assert result[0][1][0] == "test.txt"
        assert result[0][1][1] == b"test content from Path"
        assert "text/plain" in result[0][1][2]
...
class TestUpdateParamsForAuth:
    ...
    def test_update_params_for_auth_header(self):
        """Test update params with header-based API key auth."""
        client = ApiClient()
        headers = {}
        queries = []
        auth_setting = {
